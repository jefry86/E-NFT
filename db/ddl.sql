...
CREATE TABLE `nft_banner` (
  ...
  `type` tinyint(4) unsigned DEFAULT '1' COMMENT '类型 1、自营平台 2、转售平台',
  `dt_create` int(11) unsigned DEFAULT '0' COMMENT '创建时间',
  `dt_update` int(11) unsigned DEFAULT '0' COMMENT '跟新时间',
  PRIMARY KEY (`id`),
  KEY `type` (`type`)
) ENGINE=InnoDB AUTO_INCREMENT=2 DEFAULT CHARSET=utf8mb4 COMMENT='Banner list';
...
CREATE TABLE `nft_mall_goods` (
  ...
  `detail` varchar(255) DEFAULT NULL COMMENT '藏品介绍',
  ...
  `type` tinyint(4) DEFAULT NULL COMMENT '类型 1普通 2盲盒 ',
  `status` tinyint(4) unsigned DEFAULT '1' COMMENT '是否可售 1是 0下架 2 已售',
  `user_id` varchar(20) DEFAULT NULL COMMENT '发布的用户ID',
  `dt_create` int(11) unsigned DEFAULT '0' COMMENT '创建时间',
  `dt_update` int(11) unsigned DEFAULT '0' COMMENT '更新时间',
  PRIMARY KEY (`id`),
  KEY `user_id` (`user_id`)
...
CREATE TABLE `nft_mall_platform` (
  `id` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `app_key` varchar(20) DEFAULT NULL COMMENT 'app_key',
  `app_secret` varchar(50) DEFAULT NULL COMMENT 'app_secret',
  ...
  `has_online` tinyint(4) unsigned DEFAULT '0' COMMENT '是否上线 0未上线 1已上线',
...
-- ----------------------------
-- Table structure for nft_mall_platform_account
-- ----------------------------
DROP TABLE IF EXISTS `nft_mall_platform_account`;
CREATE TABLE `nft_mall_platform_account` (
  `id` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `user_id` varchar(20) DEFAULT NULL COMMENT '用户编号',
  `platform_wallet_hash` varchar(255) DEFAULT NULL COMMENT '钱包地址',
  `platform_user_id` varchar(50) DEFAULT NULL COMMENT '平台用户ID',
  `platform_id` int(11) DEFAULT NULL COMMENT '平台ID',
  `status` tinyint(4) DEFAULT '1' COMMENT '是否可用',
...
CREATE TABLE `nft_mall_user_goods` (
  ...
  `mall_platform_id` int(10) unsigned DEFAULT '0' COMMENT '平台ID',
  `user_id` varchar(20) DEFAULT NULL COMMENT '用户编号',
