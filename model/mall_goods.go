...
type NftMallGoods struct {
	...
	Platform      NftMallPlatform `json:"-" gorm:"foreignKey:PlatformID;references:id"`
	Base
}
...
func (m *NftMallGoods) List(keyword string, goodsType int, platformIds []int, status []int, sortColumn string, sort string, offset, size int) (*[]NftMallGoods, error) {
	var list []NftMallGoods
	db := m.Table().
		Preload("Platform").
		Where("status in ?", status)
	if goodsType > 0 {
		db.Where("type=?", goodsType)
	}
	if len(platformIds) > 0 {
		db.Where("platform_id IN ?", platformIds)
	}
	if len(keyword) > 0 {
		db.Where("name like ?", "%"+keyword+"%")
	}
	err := db.Offset(offset).Limit(size).
		Order(fmt.Sprintf("%s %s,status ASC", sortColumn, sort)).Find(&list).Error
	if err != nil {
		return nil, err
	}
	return &list, nil
...
func (m *NftMallGoods) Info(id int, userId string) (*NftMallGoods, error) {
	var info NftMallGoods
	db := m.Table().Preload("Platform").Where("id=?", id)
	if userId != "" {
...
func (m *NftMallGoods) Count(keyword string, goodsType int, platformIds []int, status []int) (int64, error) {
	var count int64
	db := m.Table()
	if goodsType > 0 {
		db.Where("type=?", goodsType)
	...
	err := db.Where("status in ?", status).Count(&count).Error
	return count, err
}
